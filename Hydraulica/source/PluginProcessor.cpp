...
static DSP::Core::HydraulicaParams loadParams (juce::AudioProcessorValueTreeState& apvts)
{
    DSP::Core::HydraulicaParams p;
    ...
    p.doom            = apvts.getRawParameterValue ("DOOM")->load();
    p.threshold       = apvts.getRawParameterValue ("THRESHOLD")->load();
    ...
    p.knee            = apvts.getRawParameterValue ("KNEE")->load();
    p.makeup          = apvts.getRawParameterValue ("MAKEUP")->load();
    p.scHpf           = apvts.getRawParameterValue ("SC_HPF")->load();
    p.limiterOn       = apvts.getRawParameterValue ("LIMITER_ON")->load() >= 0.5f;
    p.limiterCeiling  = apvts.getRawParameterValue ("LIMITER_CEILING")->load();
    return p;
}
